...
.siswa-kelas-big { color:var(--blue); font-size:0.82rem; font-weight:600; margin-top:2px; }
.info-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(180px,1fr)); gap:0.875rem; margin-top:1rem; }
...
.rekap-item.i { border-left:3px solid var(--blue); }
.rekap-item.s { border-left:3px solid var(--yellow); }
...
.kal-head.weekend { color:var(--red); }
...
.kal-cell.today .kal-tgl { color:var(--blue); font-weight:700; background:var(--blue-bg); border-radius:50%; }
...
.kal-ket.izin  { background:var(--blue-bg); color:var(--blue); }
.kal-ket.sakit { background:var(--yellow-bg); color:var(--yellow); }
...
table.detail-table tbody tr.row-izin  { border-left:3px solid var(--blue); }
table.detail-table tbody tr.row-sakit { border-left:3px solid var(--yellow); }
...
.ket-badge.izin  { background:var(--blue-bg); color:var(--blue); border:1px solid rgba(79,142,247,0.25); }
.ket-badge.sakit { background:var(--yellow-bg); color:var(--yellow); border:1px solid rgba(245,158,11,0.25); }
...
<a href="/cek<?= ($GLOBALS['_SERVER']['HTTP_REFERER'] ?? '') ? '' : '' ?>" class="btn-app btn-ghost mb-3" style="display:inline-flex;">
    <i class="fa-solid fa-arrow-left"></i> Kembali
</a>

<!-- Status hari ini -->
<?php if ($hariIni): ?>
<div style="background:var(--green-bg);border:1px solid rgba(34,197,94,0.25);border-radius:var(--radius);padding:0.75rem 1.25rem;margin-bottom:1rem;display:flex;align-items:center;gap:0.75rem;">
    <i class="fa-solid fa-circle-check" style="color:var(--green);font-size:1.25rem;"></i>
    <div>
        <div style="font-weight:600;font-size:0.9rem;">Sudah presensi hari ini</div>
        <div style="font-size:0.78rem;color:var(--text2);">
            <?= ucfirst($hariIni['ket']) ?> · <?= date('H:i', strtotime($hariIni['timestamp'])) ?> WIB
        </div>
    </div>
</div>
<?php else: ?>
<div style="background:var(--red-bg);border:1px solid rgba(239,68,68,0.2);border-radius:var(--radius);padding:0.75rem 1.25rem;margin-bottom:1rem;display:flex;align-items:center;gap:0.75rem;">
    <i class="fa-solid fa-clock" style="color:var(--red);font-size:1.25rem;"></i>
    <div>
        <div style="font-weight:600;font-size:0.9rem;">Belum presensi hari ini</div>
        <div style="font-size:0.78rem;color:var(--text2);">Presensi via WhatsApp sesuai panduan</div>
    </div>
</div>
<?php endif; ?>
...
<div class="info-card">
    ...
    <div class="info-grid">
        ...
        <div class="info-item">
            <div class="lbl">Pembimbing</div>
            <div class="val">
                <?= htmlspecialchars($siswa['nama_pembimbing']??'-') ?>
                <?php if (!empty($siswa['nohp_pembimbing'])): ?>
                <?php $waPb = preg_replace('/^0/','62',preg_replace('/[^0-9]/','', $siswa['nohp_pembimbing'])); ?>
                <a href="https://wa.me/<?= $waPb ?>" target="_blank" style="color:var(--green);font-size:0.75rem;margin-left:6px;">
...
<div class="rekap-bar">
    ...
    <div class="rekap-item i"><div class="num" style="color:var(--blue)"><?= $rekap['izin'] ?></div><div class="lbl"><i class="fa-solid fa-clock me-1"></i>Izin</div></div>
    <div class="rekap-item s"><div class="num" style="color:var(--yellow)"><?= $rekap['sakit'] ?></div><div class="lbl"><i class="fa-solid fa-heart-pulse me-1"></i>Sakit</div></div>
...
<div class="view-toggle">
    <a href="/cek/<?= urlencode($siswa['nis']) ?>?view=kalender" class="<?= $viewMode==='kalender'?'active':'' ?>">
        <i class="fa-solid fa-calendar-days"></i> Kalender
    </a>
    <a href="/cek/<?= urlencode($siswa['nis']) ?>?view=tabel" class="<?= $viewMode==='tabel'?'active':'' ?>">
        <i class="fa-solid fa-table-list"></i> Riwayat
    </a>
</div>
    ...
    <?php foreach ($bulanPeriode as $ym):
        ...
        for ($d=1; $d<=$jumlahHari; $d++) {
            ...
            if (isset($presensiKalender[$tgl])) {
                ...
                if ($k==='masuk') $rM++; elseif ($k==='izin') $rI++;
                elseif ($k==='sakit') $rS++; elseif ($k==='libur') $rL++;
            }
        }
    ?>
    <div class="bulan-card">
        <div class="bulan-header">
            <div class="bulan-nama"><?= namaBulanCek($ym) ?></div>
            <div class="bulan-rekap">
                <?php if ($rM>0): ?><span style="background:var(--green-bg);color:var(--green);"><?= $rM ?>× Masuk</span><?php endif; ?>
                <?php if ($rI>0): ?><span style="background:var(--blue-bg);color:var(--blue);"><?= $rI ?>× Izin</span><?php endif; ?>
                <?php if ($rS>0): ?><span style="background:var(--yellow-bg);color:var(--yellow);"><?= $rS ?>× Sakit</span><?php endif; ?>
                <?php if ($rL>0): ?><span style="background:var(--bg3);color:var(--text2);"><?= $rL ?>× Libur</span><?php endif; ?>
                <?php if ($rM+$rI+$rS+$rL===0): ?><span style="color:var(--text3);font-size:0.7rem;">Tidak ada presensi</span><?php endif; ?>
            </div>
        </div>
        ...
        <div class="kal-body">
            ...
            <div class="<?= $cls ?>">
                ...
                <a href="<?= htmlspecialchars($p['link']) ?>" target="_blank" class="kal-foto"><i class="fa-solid fa-image"></i></a>
                <?php endif; ?>
                <?php endif; ?>
            </div>
            <?php endfor; ?>
            <?php $ht=(int)date('N',strtotime("$ym-$jumlahHari")); for($i=$ht;$i<7;$i++): ?><div class="kal-cell empty"></div><?php endfor; ?>
        </div>
    </div>
    <?php endforeach; ?>
</div>
...
<div class="tabel-wrap">
    ...
    <div style="overflow-x:auto;">
        ...
        <?php else: ?>
        <table id="tabelRiwayat" class="detail-table">
            <thead><tr><th>No</th><th>Tanggal</th><th>Hari</th><th>Waktu</th><th>Status</th><th>Catatan</th><th>Foto</th></tr></thead>
            <tbody>
                ...
                <tr class="row-<?= $ket ?>">
                    ...
                    <td style="color:var(--text2);font-size:0.8rem;"><?= namaHariCek(date('Y-m-d',$ts)) ?></td>
                    ...
                    <td style="font-size:0.82rem;color:var(--text2);max-width:220px;"><?= htmlspecialchars($p['catatan']??'-') ?></td>
                    <td>
                        <?php if (!empty($p['link']) && $p['statuslink']==='OK'): ?>
